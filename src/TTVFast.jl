...
#  const global LIBTTV = Libdl.find_library(["libttvfast.so"],[".","/usr/local/lib","../c_version/"])
  #these used to have "const" in front of each "global". I have not yet found a suitable replacement for these in v1^M
  global LIBTTV = find_library(["libttvfast.so"],[".","/usr/local/lib","../c_version/"]) #removed "Libdl." in front of "find_library"^M
  #global LIBTTV = Libdl.find_library(["/Users/ericagol/Software/TTVFast/jl_version/libttvfast.so"],["/Users/ericagol/Software/TTVFast/jl_version/","/usr/local/lib","/Users/ericagol/Software/TTVFast/c_version/"])
  global ttvfast_NA_DEFAULT = convert(Cdouble,-2.0); # value for transit information that is not determined by TTVFast
  global TTVFAST_INITIALIZED = true
end
...
mutable struct ttvfast_outputs_type
...
mutable struct ttvfast_inputs_type
  param::Ptr{Cdouble}
  time_step::Cdouble
  t_start::Cdouble
  t_stop::Cdouble
...
function ttvfast_outputs_type(nevents::Integer, rv_times::Vector{Float64} = Array{Float64}(undef,0))
  @assert(nevents>=1);
  ...
  for i in 1:nrvs
    ...
    unsafe_wrap(Array,convert(Ptr{ttvfast_RV_entry},rv_workspace),nrvs)[i] = ttvfast_RV_entry(rv_times[i],0.)
  end
...
function get_event(data::ttvfast_outputs_type, i::Integer)
  ...
  unsafe_wrap(Array,data.transit_workspace,data.max_num_events)[i]
end
...
function get_rv(data::ttvfast_outputs_type, i::Integer, convert_to_mps::Bool=true)  
  ...
  rv = unsafe_wrap(Array,data.rv_workspace,data.num_rv_obs)[i].RV
  if(convert_to_mps)
...
function ttvfast!(in::ttvfast_inputs_type, out::ttvfast_outputs_type)
  ...
  LIBTTVhandle=dlopen(LIBTTV)
  CTTVFastpointer=dlsym(LIBTTVhandle,:TTVFast)

  ccall( CTTVFastpointer, Nothing, (Ptr{Nothing},Cdouble,Cdouble,Cdouble,Cint,Ptr{Nothing},Ptr{Nothing},Cint,Cint,Cint,),
...
#  ccall( (:TTVFast, LIBTTV ), Nothing, (Ptr{Nothing},Cdouble,Cdouble,Cdouble,Cint,Ptr{Nothing},Ptr{Nothing},Cint,Cint,Cint,),
#                  in.param,in.time_step,in.t_start,in.t_stop,in.num_planets,
#                  out.transit_workspace,out.rv_workspace,out.num_rv_obs,out.max_num_events,in.input_flag )
end
